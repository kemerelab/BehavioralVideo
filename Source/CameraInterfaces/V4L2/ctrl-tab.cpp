...
void SupportedCamera::addWidget(QGridLayout *grid, QWidget *w, Qt::Alignment align = Qt::AlignVCenter)
{
...
void SupportedCamera::addControlWidgets()
{
	...
	for (ClassMap::iterator iter = m_classMap.begin(); iter != m_classMap.end(); ++iter) {
        ...
        QWidget *t = new QWidget(m_tabs); //wrapper widget for set of controls

        QGroupBox *gb;
        if (QString((const char*)(qctrl.name)).isEmpty())
            gb = new QGroupBox("Controls",t);
        else
            gb = new QGroupBox(QString((const char*)(qctrl.name)),t);
        m_mainLayout->addWidget(gb,m_mainLayout->rowCount(),0);
        ...
        gb->setLayout(vbox);
        gb->setStyleSheet("QGroupBox {border: 2px solid gray;}");
	}
}

void SupportedCamera::finishGrid(QGridLayout *grid, unsigned ctrl_class)
{
...
void SupportedCamera::addCtrl(QGridLayout *grid, const v4l2_queryctrl &qctrl)
{
...
void SupportedCamera::ctrlAction(int id)
{
...
QString SupportedCamera::getString(unsigned id)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
...
long long SupportedCamera::getVal64(unsigned id)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
	QWidget *w = m_widgetMap[qctrl.id];
	long long v = 0;
...
int SupportedCamera::getVal(unsigned id)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
	QWidget *w = m_widgetMap[qctrl.id];
...
void SupportedCamera::updateCtrl(unsigned id)
{
	unsigned ctrl_class = V4L2_CTRL_ID2CLASS(id);
	if (ctrl_class == V4L2_CID_PRIVATE_BASE)
		ctrl_class = V4L2_CTRL_CLASS_USER;
...
void SupportedCamera::refresh(unsigned ctrl_class)
{
	if (!m_haveExtendedUserCtrls && ctrl_class == V4L2_CTRL_CLASS_USER) {
		for (unsigned i = 0; i < m_classMap[ctrl_class].size(); i++) {
			unsigned id = m_classMap[ctrl_class][i];
...
void SupportedCamera::refresh()
{
	for (ClassMap::iterator iter = m_classMap.begin(); iter != m_classMap.end(); ++iter)
		refresh(iter->first);
}

void SupportedCamera::setWhat(QWidget *w, unsigned id, const QString &v)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
	QString what;
...
void SupportedCamera::setWhat(QWidget *w, unsigned id, long long v)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
	QString what;
	QString flags = getCtrlFlags(qctrl.flags);
...
void SupportedCamera::setVal(unsigned id, int v)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
	v4l2_querymenu qmenu;
	QWidget *w = m_widgetMap[qctrl.id];
...
void SupportedCamera::setVal64(unsigned id, long long v)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
	QWidget *w = m_widgetMap[qctrl.id];
...
void SupportedCamera::setString(unsigned id, const QString &v)
{
	const v4l2_queryctrl &qctrl = m_ctrlMap[id];
	QWidget *w = m_widgetMap[qctrl.id];
...
void SupportedCamera::setDefaults(unsigned ctrl_class)
{
	for (unsigned i = 0; i < m_classMap[ctrl_class].size(); i++) {
		unsigned id = m_classMap[ctrl_class][i];
...
QString SupportedCamera::getCtrlFlags(unsigned flags)
{
	QString s;
